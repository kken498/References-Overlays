...
class Load_References_OT(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        for file_elem in self.files:
            ...
            # FIX: Ensure the new image is visible in the current viewport
            current_transform = references_overlays.get_image_transform_state(context, new_index)
            if current_transform:
                current_transform.hide = False
...
class Add_References_OT(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # FIX: Ensure the new slot is visible in the current viewport
        current_transform = references_overlays.get_image_transform_state(context, new_index)
        if current_transform:
            current_transform.hide = False
...
class Paste_References_OT(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if isinstance(image, Image.Image):
            ...
            # FIX: Ensure the pasted image is visible in the current viewport
            current_transform = references_overlays.get_image_transform_state(context, new_index)
            if current_transform:
                current_transform.hide = False
...
class List_Move_References_OT(bpy.types.Operator):
	bl_idname = "uilist.list_move_reference"
	bl_label = "Move Reference in List"
	bl_description = "Move the selected reference up or down in the list"
	bl_options = {"REGISTER", "UNDO"}
	...
	active_index_path : bpy.props.StringProperty(
		name="Active Index Path"
		)
	...
	def execute(self, context):
		if not self.swap:
			prop = get_reference_prop(context)
			from . import references_overlays
			transform = references_overlays.get_image_transform_state(context, prop.reference_index)

			if self.direction == "UP" and prop.reference_index > 0:
				tgt_transform = references_overlays.get_image_transform_state(context, prop.reference_index - 1)
				if transform and tgt_transform:
					values = {
						"x": transform.x,
						"y": transform.y,
						"size": transform.size,
						...
						"depth_set": transform.depth_set,
						"orthographic": transform.orthographic,
						...
						"lock_position": transform.lock_position,
					}
					transfer_properties(transform, tgt_transform)
					for key, value in values.items():
						setattr(tgt_transform, key, value)

			elif self.direction == "DOWN" and prop.reference_index < len(prop.reference) - 1:
				tgt_transform = references_overlays.get_image_transform_state(context, prop.reference_index + 1)
				if transform and tgt_transform:
					values = {
						"x": transform.x,
		...
		bpy.ops.uilist.entry_move(direction=self.direction, list_path = self.list_path, active_index_path = self.active_index_path)
